...
public class StockRestApi implements IStockRestApi{
	...
	@Override
	public Double tickerlast(String symbol) throws HttpException, IOException {
		HttpUtilManager httpUtil = HttpUtilManager.getInstance();
		String param = "";
		if(!StringUtil.isEmpty(symbol )) {
			if (!param.equals("")) {
		...
		String result = httpUtil.requestHttpGet(url_prex, TICKER_URL, param);
		return Double.parseDouble(JSON.parseObject(result, Ticker.class).getTicker().getLast());		
